...
if (!nexacro.RadarChart) {
	...
	_pRadarChart._setDatapoint = function () {
		var seriesset = this.seriesset, s, categoryaxis, pointshape;
		categoryaxis = this.categoryaxis;
	...
	_pRadarChart._arrangeAxis = function () {
		var boardRect = this._boardRect, left = 0, top = 0, width = 0, height = 0, rearrange = this._rearrange, categoryaxis = this.categoryaxis, valueaxis = this.valueaxes[0];
	...
	_pRadarChart._setAxislabelChangeInBoardAreaPos = function (item) {
		if (item && item._type_name == "GraphicsText") {
			var labelBoundRect = item.getGlobalBoundRect(), rectx = this._boardRectLeft, recty = this._boardRectTop, rectwidth = this._boardWidth + rectx, rectheight = this._boardHeight + recty, textW = labelBoundRect.width, textH = labelBoundRect.height, pos = item.getCenter();
			var cx = pos.x;
			var cy = pos.y;
...
if (!nexacro.ChartRadarCategoryAxisControl) {
	...
	_pChartRadarCategoryAxisControl._setData = function (clientWidth, clientHeight) {
		var changedTicks = this._changedTicks, axislinestyle = this._axislinestyle, axislineopacity = this._axislineopacity, boardlinestyle = this._boardlinestyle, boardlineopacity = this._boardlineopacity;
	...
	_pChartRadarCategoryAxisControl._arrangeTickGroup = function (width, height) {
		var chart = this.parent, ticks = this._ticks, value, tickmin = this._min, tickmax = this._max, axisLine, centerX = chart._centerLeft, centerY = chart._centerTop, categoryaxis = this, axislinetype = chart.radartype || "polygon", categoryaxislinewidth = 1, valueaxis = chart.valueaxes[0];
	...
	_pChartRadarCategoryAxisControl._arrangeLabelGroup = function () {
		var ticks = this._ticks, chart = this.parent, tick, labelEle, textW = 0, textH = 0, x = 0, y = 0, value, labelgap = +this._labelgap, centerX = chart._centerLeft, centerY = chart._centerTop, categoryaxis = this, valueaxis = chart.valueaxes[0], startangle, ptPos, originpoint, vPoint = {
			x : 0, 
			y : 0
		}, vUnit, rotateX = 0, rotateY = 0, targetX = 0, targetY = 0, categoryaxislinewidth = 1, valueaxisboardlinewidth = 1, axislinetype = chart.radartype || "polygon", labelBoundRect;
		...
		function positionlefttop (itemText, cx, cy, rotate, width) {
			...
			var labelBoundRect = itemText.getGlobalBoundRect(), textW = labelBoundRect.width, textH = labelBoundRect.height;
	...
	_pChartRadarCategoryAxisControl._setTransformationHelpers = function () {
		var chart = this.parent, maxRadius = Math.min(chart._boardWidth, chart._boardHeight) / 2, radius = 0, w = 0, centerX = chart._centerLeft, s, m, range, tickmin = chart.valueaxes[0]._min, tickmax = chart.valueaxes[0]._max;
...
if (!nexacro.ChartRadarValueAxisControl) {
	...
	_pChartRadarValueAxisControl._arrangeTickGroup = function () {
		var chart = this.parent, ticks = this._ticks, value, maxRadius = Math.min(chart._boardWidth, chart._boardHeight) / 2, tickmin = this._min, tickmax = this._max, axisLine, centerX = chart._centerLeft, centerY = chart._centerTop, categoryaxis = chart.categoryaxis, range, axislinetype = chart.radartype || "polygon", tickposition = this._axislineindex || 0, categoryaxislinewidth = 1, valueaxislinewidth = 1, valueaxisboardlinewidth = 1, angledata, valueaxis = this;
	...
	_pChartRadarValueAxisControl._arrangeLabelGroup = function () {
		var ticks = this._ticks, chart = this.parent, tick, labelEle, value, maxRadius = Math.min(chart._boardWidth, chart._boardHeight) / 2, labelgap = +this._labelgap, centerX = chart._centerLeft, centerY = chart._centerTop, categoryaxis = chart.categoryaxis, startangle, range, originpoint, targetX = 0, targetY = 0, tickposition = this._axislineindex || 0, valueaxislinewidth = 1;
	...
	_pChartRadarValueAxisControl._setTransformationHelpers = function () {
		var chart = this.parent, maxRadius = Math.min(chart._boardWidth, chart._boardHeight) / 2, radius = 0, h = 0, centerY = chart._centerTop, s, m, tickmin = this._min, range, tickmax = this._max;
...
if (!nexacro.ChartRadarSeriesControl) {
		...
		if (isselectitem) {
			...
			if (area) {
				if (lineareavisible) {
					...
					if (!nexacro._ChartGraphicsLib.isEmpty(selectlineareaopacity)) {
	...
	_pChartRadarSeriesControl._hideLineAreaHighlight = function (item) {
		...
		var lineareaHighlight = item._lineareaHighlight;
		if (lineareaHighlight && !nexacro._ChartGraphicsLib.isEmpty(lineareaHighlight.parent)) {
			this._chart._chageGroupObject(this._chart._seriesGroup, this._chart._highlightGroup, this._itemtextlist, true);
	...
	_pChartRadarSeriesControl._drawLineAreaHighlight = function (item) {
		...
		if (!nexacro._ChartGraphicsLib.isEmpty(highlightlineareaopacity)) {
