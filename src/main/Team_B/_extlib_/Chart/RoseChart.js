...
if (!nexacro.RoseChart) {
	...
	_pRoseChart.categoryaxis = null;
	_pRoseChart.valueaxes = null;
...
if (!nexacro.ChartRoseCategoryAxisControl) {
	...
	_pChartRoseCategoryAxisControl._arrangeTickGroup = function (width, height) {
		var chart = this.parent, ticks = this._ticks, value, tickmin = this._min, tickmax = this._max, axisLine, centerX = chart._centerLeft, centerY = chart._centerTop, categoryaxis = this, categoryaxislinewidth = 1, valueaxis = chart.valueaxes[0];
	...
	_pChartRoseCategoryAxisControl._arrangeLabelGroup = function () {
		var ticks = this._ticks, chart = this.parent, tick, labelEle, textW = 0, textH = 0, x = 0, y = 0, value, maxRadius = Math.min(chart._boardWidth, chart._boardHeight) / 2, labelgap = +this._labelgap, centerX = chart._centerLeft, centerY = chart._centerTop, categoryaxis = this, valueaxis = chart.valueaxes[0], radius, startangle, ptPos, originpoint, vPoint = {
			x : 0, 
			y : 0
		}, vUnit, rotateX = 0, rotateY = 0, targetX = 0, targetY = 0, categoryaxislinewidth = 1, valueaxislinewidth = 1, valueaxisboardlinewidth = 1, labelBoundRect;
...
if (!nexacro.ChartRoseValueAxisControl) {
	...
	_pChartRoseValueAxisControl.axislineindex = "0";
	...
	_pChartRoseValueAxisControl._setData = function (clientWidth, clientHeight) {
		...
		var labelPos = this.labeltextposition;
	...
	_pChartRoseValueAxisControl._arrangeLabelGroup = function () {
		...
		for (var i = 0; i < ticks.length; i++) {
			...
			for (var j = 0; j < 2; j++) {
				...
				if (nexacro._ChartGraphicsLib.isEmpty(labelEleItm)) {
					continue;
				}
...
if (!nexacro.ChartRoseSeriesControl) {
	...
	_pChartRoseSeriesControl._applyPropertySeries = function (type, style, value, select) {
		...
		if (seriesGroup) {
			for (var i = 1; i <= this._itemCnt; i++) {
				var itemID = this._configIndex + " SeriesRoseItem_" + (i - 1), isselectitem = false;
	...
	_pChartRoseSeriesControl._drawSeriesLines = function () {
		var linestyle = this.linestyle || "1px solid " + this._color, categoryaxis = this._chart.categoryaxis, valueaxis = this._chart.valueaxes[0], path, area, datapoints = this._datapoints, seriesGroup = this._chart._seriesGroup, effect = this._chart_aniframe_obj, length = 0;
	...
	_pChartRoseSeriesControl._drawHighlight = function (item) {
		var highlightlinestyle = this._highlightlinestyle || "1px solid " + this._chart._highlightcolorset[0], highlightfillstyle = this._highlightfillstyle, highlightopacity = this.highlightopacity, slice, highlightPathData, highlightGroup = this._chart._highlightGroup;
	...
	_pChartRoseSeriesControl._showRoseHighlight = function (item) {
		...
		if (!item || (item.id.search("RoseItem") < 0)) {
			return;
		}
	...
	_pChartRoseSeriesControl._drawRose = function (datapoints, xoffset, yoffset, categoryaxis, valueaxis) {
		var points = datapoints.points.slice(0), ps = datapoints.pointsize, path, effect = this._chart_aniframe_obj, seriesid, seriesGroup = this._chart._seriesGroup;

		xoffset += -this._chart._boardRectLeft;
		yoffset += -this._chart._boardRectTop;

		var centerX = this._chart._centerLeft, centerY = this._chart._centerTop;
	...
	_pChartRoseSeriesControl._setColor = function (colorset) {
		...
		if (changedColorset) {
			var linevisible = this._linestyle, linestyle;
