    ...
    _pPieChart._hideSeriesItem = function (id)
    {
        ...
        if (s)
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                {
                    item._isShow = false;
					/*var lineitemid = s._configIndex + " SeriesPieItemLine_" + i;
					var textitemid = s._configIndex + " SeriesItemText_" + i;
					var lineitem = seriesGroup.getObjectByID(lineitemid);
					var textitem = seriesGroup.getObjectByID(textitemid);
					if(textitem)textitem.set_visible(item._isShow);
					if(lineitem)lineitem.set_visible(item._isShow);*/
                    this._rearrange = true;
					//this._changedData = true;
                    if (this.legend)
                    {
...
if (!nexacro.ChartPieSeriesControl)
{
	...
	_pChartPieSeriesControl.highlightfilltype = true;	// hsji 8.28
	_pChartPieSeriesControl.autogradation = "none";		// hsji 8.28
            ...
            if (items[i]._isShow)
            {
				...
				if(nexacro._Browser != "Runtime" && (pThis.autogradation != "none"))
				{
					...
					if(bTarget)
					{
						...
						// linear-gradient(to bottom,#ddffff 23%, #0000ff 100%)
						var gColor = "";
