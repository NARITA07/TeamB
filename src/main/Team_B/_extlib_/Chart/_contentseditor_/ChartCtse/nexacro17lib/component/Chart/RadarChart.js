...
if (!nexacro.RadarChart)
{
	...
	//_pRadarChart.valueaxis = null;    // readonly->inner
    ...
    //_pRadarChart.set_valueaxis = function () { };
...
/*
		if(categoryaxis)
		{
			 ...
			 axisRect = categoryaxis._axisRect._getRect(false, true);
                    gap = categoryaxis.gap || 0;
		...
		if(valueaxis)
		{
			 ...
			 axisRect = valueaxis._axisRect._getRect(false, true);
                    gap = valueaxis.gap || 0;
...
*/
	...
	_pRadarChart._arrangeAxis = function ()
    {
		...
		if (rearrange || (categoryaxis && categoryaxis._rearrange) || (valueaxis && valueaxis._rearrange) )
        {
			if (boardRect)
			{
				//this._boardWidth = boardRect.width - boardBorderWidth;
				//this._boardHeight = boardRect.height - boardBorderHeight;
	...
	/*_pRadarChart._deleteValueaxis = function ()
    {
        var valueaxis = this.valueaxis;
        if (valueaxis)
        {
            this._deleteAxis(valueaxis, false);
            this._changedData = true;
        }
    };
	*/
...
if (!nexacro.ChartRadarValueAxisControl)
{
    ...
    // 10월 정기 이후에 기능 변경 소지 있음 - 사용X
	/*_pChartRadarValueAxisControl.set_labelgapangle = function (val)
    {
        var lVal = null;
        if (val !== undefined && val !== null && val !== "")
        {
        ...
        if (this.labelgapangle != val)
        {
            this._changeContentsProperty("labelgapangle", val, this.labelgapangle);
            this.labelgapangle = val;
			this._labelgap = +val;
            this.on_apply_labelgapangle();
        }

        this.parent._draw();
    };
	_pChartAxisControl.on_apply_labelgapangle = function ()
    {
    ...
    */
	...
	/*_pChartRadarValueAxisControl.set_axislinetype = function(val)
	{
		var axislinetype_enum = ["polygon","circle"];
		if (axislinetype_enum.indexOf(val) == -1)
        {
        ...
        if (this.axislinetype != val)
        {
			this._changeContentsProperty("axislinetype", val, this.axislinetype);
            this.axislinetype = val;
	...
	_pChartRadarValueAxisControl.on_apply_axislinetype = function()
	{
		this._changedTicks = true;
		this._rearrange = true;
...
if (!nexacro.ChartRadarSeriesControl)
{
    ...
    _pChartRadarSeriesControl._applyPropertySeries = function (type, style, value, select)
   {
	...
	_pChartRadarSeriesControl._drawPointItemText = function (cx, cy, item)
    {
        ...
        itemtextPosition = this.pointitemtextposition,
